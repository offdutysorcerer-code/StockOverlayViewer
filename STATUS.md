...
|---|---|---|---|
| ChatGPT | DONE | `scripts/StartDataCollector.ps1`, `frontend/js/app.js`, `data/groups.json`, `STATUS.md` | Changed US/non-mock Start Feed interval to 5 seconds and changed `StartDataCollector.ps1` default `IntervalSeconds` to 5. Collector now reloads provider symbols from `groups.json` on every iteration, so newly added US symbols are picked up without restarting the collector. Added GLW to the US MVP group and verified `StartDataCollector.ps1 -Provider us -Once` writes `data/intraday/GLW.json` and `data/daily/GLW.json` from Yahoo Finance. |
